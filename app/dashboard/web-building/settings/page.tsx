  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        const response = await fetch('/api/webvault/settings', {
          headers: {
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="secondary">
                  {stats?.platformStats?.activeWebsites || 0}
                </Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Total Storage</span>
                <Badge variant="secondary">
                  {stats?.platformStats?.totalStorage ? `${(stats.platformStats.totalStorage / 1024).toFixed(1)} GB` : '0 GB'}
                </Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">SSL Certificates</span>
                <Badge variant="secondary">
                  {stats?.platformStats?.sslCertificates || 0}/{stats?.platformStats?.activeWebsites || 0}
                </Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Uptime</span>
                <Badge className="bg-green-100 text-green-800">
                  {stats?.platformStats?.uptime?.toFixed(1) || 99.9}%
                </Badge>
              </div>
            </CardContent>
          </Card>
