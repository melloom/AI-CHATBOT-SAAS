...
export default function Services() {
  ...
  // Define the catalog services array
  ...
  const handleAddFromCatalog = async () => {
    try {
      const servicesToSubmit = catalogServices.filter(service => selectedCatalogServices.includes(service.id))
        ...
        {filteredServices.length === 0 ? (
          <div className="col-span-full text-center py-12">
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-8">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">No services match your current filters.</p>
              <Button onClick={() => {
                setSearchTerm("")
                ...
                <div className="flex items-center justify-between">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => handleShowServiceDetails(service)}>
      ...
      <Dialog open={showServiceCatalog} onOpenChange={setShowServiceCatalog}>
        <DialogContent className="max-w-6xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Browse and add services from our comprehensive catalog
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                { id: 'web-design', name: 'Web Design', icon: <Palette className="h-5 w-5" />, count: 6 },
                { id: 'development', name: 'Development', icon: <Code className="h-5 w-5" />, count: 6 },
                { id: 'hosting', name: 'Hosting', icon: <Server className="h-5 w-5" />, count: 5 },
                { id: 'maintenance', name: 'Maintenance', icon: <Wrench className="h-5 w-5" />, count: 5 },
                { id: 'security', name: 'Security', icon: <Shield className="h-5 w-5" />, count: 5 },
                { id: 'seo', name: 'SEO', icon: <Search className="h-5 w-5" />, count: 5 },
                { id: 'analytics', name: 'Analytics', icon: <BarChart3 className="h-5 w-5" />, count: 5 },
                { id: 'content', name: 'Content', icon: <FileText className="h-5 w-5" />, count: 5 },
                { id: 'ecommerce', name: 'E-commerce', icon: <ShoppingCart className="h-5 w-5" />, count: 5 },
                { id: 'mobile', name: 'Mobile', icon: <Smartphone className="h-5 w-5" />, count: 4 },
                { id: 'marketing', name: 'Marketing', icon: <Target className="h-5 w-5" />, count: 5 },
                { id: 'consulting', name: 'Consulting', icon: <Users className="h-5 w-5" />, count: 4 },
                { id: 'training', name: 'Training', icon: <GraduationCap className="h-5 w-5" />, count: 4 },
                { id: 'advanced', name: 'Advanced', icon: <Cpu className="h-5 w-5" />, count: 4 },
                { id: 'enterprise', name: 'Enterprise', icon: <Building2 className="h-5 w-5" />, count: 4 }
              ].map((category) => (
                <Card 
                  key={category.id} 
                  className={`hover:shadow-md transition-shadow cursor-pointer ${
                    selectedCatalogCategory === category.id ? 'ring-2 ring-blue-500' : ''
                  }`}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold">
                    Services in {catalogServices.find(s => s.category === selectedCatalogCategory)?.category.replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                  </h3>
                  <Button 
                    variant="outline" 
                    size="sm"
                    onClick={() => setSelectedCatalogCategory("")}
                  >
                    Back to Categories
                  </Button>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {catalogServices
                    .filter(service => service.category === selectedCatalogCategory)
                    .map((service) => {
                      const isSelected = selectedCatalogServices.includes(service.id)
                      return (
                        <Card 
                          key={service.id} 
                          className={`hover:shadow-md transition-all duration-200 cursor-pointer border-2 ${
                            isSelected 
                              ? 'ring-2 ring-green-500 bg-gradient-to-br from-green-50 to-emerald-50 border-green-200 shadow-lg' 
                              : 'border-gray-200 hover:border-blue-300'
                          }`}
                          onClick={() => {
                            if (isSelected) {
                              setSelectedCatalogServices(prev => prev.filter(id => id !== service.id))
                            } else {
                              setSelectedCatalogServices(prev => [...prev, service.id])
                            }
                          }}
                        >
                          <CardContent className="p-4">
                            <div className="space-y-3">
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-3">
                                  <div className={`p-2 rounded-lg transition-colors ${
                                    isSelected ? 'bg-green-100 text-green-700' : 'bg-blue-100 dark:bg-blue-900/30'
                                  }`}>
                                    {service.icon}
                                  </div>
                                  <div className="flex-1">
                                    <h4 className={`font-semibold transition-colors ${
                                      isSelected ? 'text-green-900' : 'text-gray-900 dark:text-white'
                                    }`}>
                                      {service.name}
                                    </h4>
                                    <p className={`text-sm transition-colors ${
                                      isSelected ? 'text-green-800' : 'text-gray-600 dark:text-gray-400'
                                    }`}>
                                      {service.description}
                                    </p>
                                  </div>
                                ...
                                <div className="flex items-center space-x-2">
                                  {isSelected ? (
                                    ...
                                    <span className="text-xs font-semibold text-green-900">Selected</span>
                                  </div>
                                  ) : (
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2">
                                  <Badge variant={isSelected ? "default" : "secondary"} className={
                                    isSelected ? "bg-green-200 text-green-900 border-green-400 font-semibold" : ""
                                  }>
                                  ...
                                  <Badge variant="outline" className="text-xs">
                                   ...
                                   <div className="flex items-center space-x-1">
                                     ...
                                     <span className="text-xs font-medium text-green-900">Ready to add</span>
                                   </div>
                                 )}
                              </div>
                              ...
                              {isSelected && (
                                   ...
                                   <div className="flex items-center justify-between mb-2">
                                     <span className="text-sm font-semibold text-green-900">Service Details</span>
                                     <Star className="h-4 w-4 text-yellow-600 fill-current" />
                                   </div>
                                   <div className="grid grid-cols-2 gap-2 text-xs">
                                     <div className="flex items-center space-x-1">
                                       <TrendingUp className="h-3 w-3 text-green-700" />
                                       <span className="text-green-900 font-medium">Popular choice</span>
                                     </div>
                                     <div className="flex items-center space-x-1">
                                       <CheckCircle className="h-3 w-3 text-green-700" />
                                       <span className="text-green-900 font-medium">Professional quality</span>
                                     </div>
                                   </div>
                                 </div>
                              )}
            ...
            {selectedCatalogServices.length > 0 && (
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                <h3 className="font-semibold mb-2">Selected Services ({selectedCatalogServices.length})</h3>
                <div className="flex flex-wrap gap-2">
                                     {selectedCatalogServices.map((serviceId) => {
                     const service = catalogServices.find((s: any) => s.id === serviceId)
                     return service ? (
                       <Badge key={serviceId} variant="secondary" className="flex items-center space-x-1">
                         {service.icon}
                         <span>{service.name}</span>
                         <button
                           onClick={() => setSelectedCatalogServices(prev => prev.filter(id => id !== serviceId))}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowServiceCatalog(false)}>
              Cancel
            </Button>
            <Button 
              onClick={handleAddFromCatalog}
              disabled={selectedCatalogServices.length === 0}
            >
              Add Selected Services ({selectedCatalogServices.length})
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
